...
use polars::export::arrow::array::{Array, ArrayRef, Int32Array, Int64Array, StructArray};
...
use polars::prelude::{DataFrame, NamedFrom, Result as PolarsResult, Series, TimeUnit};
use std::collections::HashMap;
use std::convert::TryFrom;
use std::io::Read;
use std::sync::Arc;

pub fn ipc_binary_to_dataframe<R: Read>(arrow_data: &mut R) -> PolarsResult<DataFrame> {
    ...
    let mut series_vec: HashMap<usize, Series> = HashMap::with_capacity(chunks.len());
        ...
        for (array_index, array) in chunk.arrays().iter().enumerate() {
            ...
            match array.data_type() {
                ...
                | ArrowDataType::Int64 => {
                    ...
                    let cloned_array: ArrayRef = array.clone();
                ...
                ArrowDataType::Struct(_f) => {
                    let logical_type = fm.get("logicalType").unwrap().as_str();
                    let _cloned_array: ArrayRef = array.clone();
                    let found_series = series_vec.get_mut(&array_index).unwrap();
